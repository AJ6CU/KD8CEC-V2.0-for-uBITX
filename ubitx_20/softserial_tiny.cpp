...
void SWSerial_Begin(long speedBaud)
{
  ...
  _receivePortRegister = portInputRegister(digitalPinToPort(SOFTWARESERIAL_RX_PIN));

  //Set Values
  uint16_t bit_delay = (F_CPU / speedBaud) / 4;
  _tx_delay = subtract_cap(bit_delay, 15 / 4);

  if (digitalPinToPCICR(SOFTWARESERIAL_RX_PIN)) 
  {
    _rx_delay_centering = subtract_cap(bit_delay / 2, (4 + 4 + 75 + 17 - 23) / 4);
    _rx_delay_intrabit = subtract_cap(bit_delay, 23 / 4);
    _rx_delay_stopbit = subtract_cap(bit_delay * 3 / 4, (37 + 11) / 4);
    *digitalPinToPCICR(SOFTWARESERIAL_RX_PIN) |= _BV(digitalPinToPCICRbit(SOFTWARESERIAL_RX_PIN));
    _pcint_maskreg = digitalPinToPCMSK(SOFTWARESERIAL_RX_PIN);
    _pcint_maskvalue = _BV(digitalPinToPCMSKbit(SOFTWARESERIAL_RX_PIN));
