  ...
  while(1)
  {
    ...
    if (nowWsprStep == 0) //select Message status
    {
      ...
      if (selectedWsprMessageIndex != nowSelectedIndex)
      {
        ...
        for (int i = 0; i < 41; i++)
        {
          byte readData = EEPROMTYPE.read(WSPR_MESSAGE1 + 5 + (wsprMessageBuffIndex) + i);  //NAME TAG 5, MESSAGE 41 = 46
    ...
    else if (nowWsprStep == 1)
    {
      ...
      if (selectedWsprBandIndex != nowSelectedIndex)
      {
        ...
        EEPROMTYPE.get(bandBuffIndex, WsprTXFreq); 
        EEPROMTYPE.get(bandBuffIndex + 4, WsprMultiChan); 

        for (loopIndex = 3; loopIndex < 8; loopIndex++)
          Wspr_Reg1[loopIndex] = EEPROMTYPE.read(bandBuffIndex + loopIndex + 3);
